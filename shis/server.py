...
def generate_thumbnail(paths: Tuple[str, str, str, str], args: argparse.Namespace):
    """Takes a tuple of paths and generates thumbnail(s).

    Processes :attr:`paths` returned from :func:`process_paths`
    and generates
...
def process_paths(args: argparse.Namespace) -> Tuple[Tuple[str, str, str, str], int]:
    """Generate thumbnail paths to be processed by :func:`generate_thumbnail`

    This function traverses ``args.image_dir``

    :param args: preprocessed command line arguments.
    :return: a 4-tuple containing the absolute path of the original 
        image, and the absolute path of the small, large and full size
        thumbnails to be generated by :func:`generate_thumbnail`.
    """
    paths = []
    num_pages = 0
...
def generate_albums(args: argparse.Namespace) -> Tuple[Dict, int]:
    ...
    This function generates the correct names and URLs for
    all thumbnails, albums, breadcrumbs and previews for each
    page of the website.

    :param args: preprocessed command line arguments.
    :return: a generator containing data required to populate each page.
    """
...
def create_templates(args: argparse.Namespace, num_pages: int) -> None:
    ...
    This function creates ``static`` and ``html`` directories
    inside ``args.thumb_dir``. If the directories already exist, 
    they will be cleared before continuing. All static content
    (JS/CSS) is stored in ``{args.thumb_dir}/static`` and all HTML
    files (except the first ``index.html``) are stored in 
    ``{args.thumb_dir}/html``. The data required to populate
    Jinja2 templates is derived from :func:`generate_albums`.
...
def preprocess_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    paths supplied for ``args.image_dir`` and ``args.thumb_dir``
...
if __name__ == '__main__':
    ...
    parser.add_argument('image_dir', nargs='?', default='',
        help='directory to look for images (default: current directory)')
    ...
    parser.add_argument('--port', type=int, default=8000,
        help='port to host the server on (default: 8000)')
    parser.add_argument('--samples', type=int, default=0,
        help='randomly sample images insted of processing all of them (default: all)')
    parser.add_argument('--thumb-size', type=int, default=320, metavar='SIZE',
        help='size of the generated thumbnails in pixels (default: 320)')
    parser.add_argument('--preview-size', type=int, default=1024, metavar='SIZE',
        help='size of large previews in pixels, if generated (default 1024)')
