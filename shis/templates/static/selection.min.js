...
(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.SelectionArea = factory());
     ...
     */
    function intersects(a, b, mode = 'touch') {
        switch (mode) {
            case 'center': {
    ...
    class SelectionArea extends EventTarget {
        constructor(opt) {
            super();
            // Selection store
            this._selection = {
            ...
            this._areaLocation = { y1: 0, x2: 0, y2: 0, x1: 0 };
        ...
        _onSingleTap(evt) {
            ...
            if (evt.shiftKey && stored.length) {
                ...
                const rangeItems = [...this._selectables.filter(el => (el.compareDocumentPosition(preceding) & 4) &&
                        (el.compareDocumentPosition(following) & 2)), target, preceding, following];
        ...
        _delayedTapMove(evt) {
            ...
            (thresholdType === 'number' && abs((x + y) - (x1 + y1)) >= startThreshold) ||
                ...
                (thresholdType === 'object' && abs(x - x1) >= startThreshold.x || abs(y - y1) >= startThreshold.y)) {
        ...
        _onTapMove(evt) {
            ...
            const scon = this._targetElement;
            _areaLocation.x2 = x;
            _areaLocation.y2 = y;
        ...
        _recalculateSelectionAreaRect() {
            ...
            const { scrollTop, scrollHeight, clientHeight, scrollLeft, scrollWidth, clientWidth } = _targetElement;
            const brect = _targetRect;
        ...
        select(query, quiet = false) {
            ...
            const elements = selectAll(query, this._options.document).filter(el => !selected.includes(el) &&
                !stored.includes(el));
            // Update stores
